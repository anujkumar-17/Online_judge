...
const addTestCase = async (req, res) => {
  try {
    ...
    const newTestCase = new TC.modelTC({
      pid: pid,
      input: input,
      output: output,
    });
    await newTestCase.save();
    res.status(200).json({ message: "Test case added successfully", data: newTestCase });
  } catch (error) {
    res.status(500).json({ message: "Something went wrong while adding test case", error: error.message });
  }
};
