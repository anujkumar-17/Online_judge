...
const createQ=async(req,res)=>{
  try{
    ...
    const existingQuestion=await modelQ.findOne({pName});
    if(existingQuestion){
        return res.status(400).json({message:"Question already exists"});
    }
    const newQuestion=await modelQ({pid, pName, statement});
...
const readQ=async(req,res)=>{
   try{
      ...
      const ques=await modelQ.find({pName});
      if(!ques){
...
const updateQ=async(req,res)=>{
    try{
      ...
      const response=await modelQ.updateOne({pName}, {$set:{statement}});
...
const deleteQ=async(req,res)=>{
  try{
    ...
    const response=await modelQ.deleteOne({pName});
    res.status(200).json({message:response})
  }catch(error){
